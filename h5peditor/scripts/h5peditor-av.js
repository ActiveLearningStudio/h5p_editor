 ...
 *
 */
H5PEditor.widgets.video =
  H5PEditor.widgets.audio =
  H5PEditor.AV =
    (function ($) {
      /**
       * Constructor.
      ...
      C.prototype.appendTo = function ($wrapper) {
        ...
        const toggleTab = function () {
          ...
          if (activeTab === TABS.UPLOAD) {
            ...
            self.$insertButton[0].disabled =
              !tabInstances[activeTab].hasMedia();
          }
        };
        ...
        $loadPlaylist.click(function () {
                  ...
                  function (success, results) {
                    if (
                      ...
                      results.objects.forEach(function (item, index) {
                        var node =
                          '<div class="play-lists" data-filter-item data-filter-name="' +
          ...
          // var loadPlaylistPromise = new Promise(function(resolve, reject) {
          //   resolve();
          function addPagination() {
            ...
            $(function () {
              var videoCountNumber;
              totalVideoCount
                ? (videoCountNumber = totalVideoCount)
                : (videoCountNumber = results);

              var numberOfItems = videoCountNumber;
              var limitPerPage = 2;
              ...
              // Show the page links
              $("#modalContent").removeClass("hide-playlist");
              showPage(1);
              ...
              $(document).on(
                ...
                function () {
                  ...
                  if ($("#input-playlist").val() == "") {
                    getPlaylistData(2, pageNumber, "");
                  }
                  if ($("#input-playlist").val() != "") {
                    var searchText = $("#input-playlist").val();
                    getPlaylistData(2, pageNumber, searchText);
                  }
                }
              );
          ...
          // loadPlaylistPromise.then(function paginateList () {
          getPlaylistData(2, 0, "");
          setTimeout(() => {
            addPagination();
          }, 3000);
        ...
        this.$insertButton = this.$addDialog
          .find(".h5p-insert")
          .click(function () {
            if (isExtension(activeTab)) {
      ...
      C.prototype.addFile = function (index) {
        ...
        var defaultQualityName = H5PEditor.t(
          "core",
          "videoQualityDefaultLabel",
          { ":index": index + 1 }
        );
        var qualityName =
      ...
      C.createTabbedAdd = function (type, extraTabs, id, hasDescription) {
        ...
        const tabs = ["BasicFileUpload", "InputLinkURL", "LoadPlaylist"];
      ...
      C.createAdd = function (type, id, hasDescription) {
        return C.createInsertDialog(
          '<div class="h5p-dialog-box">' +
            ...
            '<div class="h5p-or-vertical-word">' +
            H5PEditor.t("core", "or") +
            "</div>" +
            "</div>" +
            "</div>" +
      ...
      C.providers = [
        {
          name: "YouTube",
          regexp:
            /(?:https?:\/\/)?(?:www\.)?(?:(?:youtube.com\/(?:attribution_link\?(?:\S+))?(?:v\/|embed\/|watch\/|(?:user\/(?:\S+)\/)?watch(?:\S+)v\=))|(?:youtu.be\/|y2u.be\/))([A-Za-z0-9_-]{11})/i,
          aspectRatio: "16:9",
        },
      ];
